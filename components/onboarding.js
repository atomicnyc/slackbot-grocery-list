...
module.exports = function (controller) {
  controller.on('onboard', (bot) => {
    ...
    if (controller.config.studio_token) {
      bot.api.im.open({ user: bot.config.createdBy }, (err, direct_message) => {
        if (err) {
          debug('Error sending onboarding message:', err);
        } else {
          controller.studio.run(bot, 'onboarding', bot.config.createdBy, direct_message.channel.id, direct_message).catch((err) => {
